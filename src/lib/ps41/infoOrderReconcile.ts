...
// 樣式：22 筆「整排路燈不亮」、30 筆「status:9(此路燈已停用)」（使用者稱之
// 為「重複偵測」——燈已停用代表這個位置已經有既有案件在處理，系統才會跳過
// 不重複開單）、0 筆其他。這個模組把每一筆 Info_Order 拆成這四類，讓「系統
// 開單」旁邊可以附上對帳用的輔助數字，不用再翻原始檔案人工比對。
//
...
const DISABLED_RE = /status\s*[:：]\s*9\s*\(此路燈已停用\)/;
...
function reconcileStatusOf(notifyResult: string): InfoOrderReconcileStatus {
  ...
  return "other"; // 含少見的「案件N進行中」既有案件比對樣式、系統錯誤等
}
